...
// TILLFÄLLIG engångs-route: deployar Firestore Security Rules via Rules REST API
// med Admin SDK:ns service-account-token. Skyddad av CLEANUP_SECRET. Tas bort
// efter användning. Rules-API:t validerar källan, så ogiltiga regler avvisas.
export async function POST(req: NextRequest) {
  ...
  if (!rules || typeof rules !== "string" || rules.length < 50) {
    return NextResponse.json({ error: "Saknar regler" }, { status: 400 });
  }
  ...
  try {
    ...
    // 1. Skapa ruleset (valideras server-side).
    ...
    if (!rsRes.ok) {
      return NextResponse.json({ step: "createRuleset", error: rsJson }, { status: 502 });
    }
    const rulesetName: string = rsJson.name;

    // 2. Peka Firestore-releasen (cloud.firestore) på den nya ruleset:en.
    ...
    if (!patchRes.ok) {
      return NextResponse.json({ step: "updateRelease", error: patchJson, rulesetName }, { status: 502 });
    }
