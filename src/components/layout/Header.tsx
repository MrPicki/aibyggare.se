  ...
  return (
    <header
      className={cn(
        ...
        scrolled
          ? "border-b-2 border-ink bg-paper/95 backdrop-blur-sm shadow-[0_4px_0_0_var(--ink)]"
          : "border-b-2 border-transparent bg-cream",
      )}
    >
          ...
          <div className="hidden md:flex items-center justify-end gap-2 flex-1">
            {!loading &&
              (user ? (
                ...
                <Link
                  href="/login"
                  className="rounded-xl px-3.5 py-2 font-mono text-[13px] font-semibold uppercase tracking-wide text-ink hover:bg-paper transition-colors"
                >
                  Logga in
                </Link>
              ))}
          ...
          <nav className="mx-auto flex max-w-7xl flex-col gap-1 px-4 py-4" aria-label="Mobilnavigation">
            ...
            <div className="mt-2 flex flex-col gap-2 border-t-2 border-dashed border-border pt-3">
              {!loading && user ? (
                ...
                <Link
                  href="/login"
                  className="rounded-xl px-3 py-3 font-mono text-base font-medium uppercase tracking-wide text-ink hover:bg-paper transition-colors"
                  onClick={() => setMobileOpen(false)}
                >
                  Logga in
                </Link>
